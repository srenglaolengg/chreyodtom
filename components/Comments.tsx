...
const Comments: React.FC<CommentsProps> = ({ language, user }) => {
    ...
    return (
        <>
            ...
            />
            <section id="comments" className="py-20 bg-secondary/30">
                <div className="container mx-auto px-6">
                    <h2 className={`text-3xl md:text-4xl font-bold text-primary text-center mb-12 ${language === 'km' ? 'font-khmer' : ''}`}>
                    ...
                    <div className="max-w-3xl mx-auto bg-card p-4 sm:p-6 md:p-8 rounded-lg shadow-lg border border-border">
                        {user ? (
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
                                ...
                                <button onClick={handleLogout} className="bg-secondary text-secondary-foreground px-4 py-2 rounded-md hover:bg-secondary/80 transition-colors text-sm font-semibold self-start sm:self-center flex-shrink-0">
                                    {currentContent.logout}
                                </button>
                            </div>
                        ) : (
                            <div className="text-center mb-6">
                                <button onClick={handleLogin} className="inline-flex items-center space-x-3 bg-gray-800 text-white px-6 py-3 rounded-md hover:bg-gray-900 dark:bg-gray-200 dark:text-gray-900 dark:hover:bg-gray-300 transition-colors shadow-md font-semibold">
                        ...
                        {user && (
                            <form onSubmit={handleSubmitComment} className="mb-8">
                                <textarea
                                    ...
                                    className={`w-full p-4 border border-input bg-transparent rounded-md focus:ring-2 focus:ring-ring focus:border-ring transition-colors ${language === 'km' ? 'font-khmer' : ''}`}
                                    rows={4}
                                    aria-label="New comment"
                                />
                                <button type="submit" className={`mt-4 w-full bg-primary text-primary-foreground px-6 py-3 rounded-md hover:bg-primary/80 transition-colors shadow-md font-semibold disabled:opacity-50 disabled:cursor-not-allowed ${language === 'km' ? 'font-khmer' : ''}`} disabled={!newComment.trim()}>
                        ...
                        <div className="space-y-6">
                            ...
                            {!loading && comments.length === 0 && <p className="text-center text-muted-foreground">{currentContent.noComments}</p>}
                            {!loading && comments.map(comment => (
                                <article key={comment.id} className="flex items-start space-x-4 p-4 bg-muted/50 rounded-lg" aria-label={`Comment by ${comment.user.displayName}`}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className={`text-foreground/90 whitespace-pre-line break-words ${language === 'km' ? 'font-khmer' : ''}`}>{comment.text}</p>
                                    </div>
                                </article>
                            ))}
                        </div>
